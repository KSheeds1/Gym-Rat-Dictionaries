...
Logged in users can 'upvote' or 'Downvote' a definition by simply clicking on the thumbs up or down icon featured on each individual definition card panel, the amount of votes for both are displayed on the icon and increment by one when clicked.

![Up/Down vote](static/images/TESTING/UST-up-down-vote.png)
 
* **UST6: *Edit or delete any definition I have added:***

A logged in user can choose to edit or delete any of their own added definitions. To do so, they can simply navigate to their profile from the navigation bar and view their 'My Definitions' or, by searching for the name of the definition using the search feature and clicking on the edit or delete button in the search results. Each card panel has two buttons, 'Edit' & 'Delete'. 
